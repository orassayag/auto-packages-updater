...
class ProjectService {
  ...
  async createProjects(resultData) {
    ...
    this.projectsDataModel.projectsList = this.projectsDataModel.projectsList.reduce(
      (acc, item) => {
  ...
  validateProjects() {
    ...
    if (
      this.projectsDataModel.projectsList.length >
      countLimitService.countLimitDataModel.maximumProjectsCount
    ) {
      this.projectsDataModel.projectsList = this.projectsDataModel.projectsList.slice(
        0,
        countLimitService.countLimitDataModel.maximumProjectsCount
      );
    }
  ...
  compareProjects(projectDataModel) {
    ...
    for (let i = 0; i < this.projectsDataModel.projectsList.length; i++) {
      ...
      if (
        projectDataModel.id === currentProjectDataModel.id ||
        currentProjectDataModel.status !== ProjectStatusEnum.CREATE
      ) {
        continue;
      }
  ...
  async validateCreateProject(data) {
    ...
    projectDataModel.displayName = `${projectDataModel.name}${
      projectDataModel.displayName ? `: ${projectDataModel.displayName}` : ''
    }`;
  ...
  async validateCustomPackagesPath(data) {
    ...
    projectDataModel.customPackagesList = resultData
      .split('\r\n')
  ...
  validateExcludePackagesList(data) {
    ...
    if (validationUtils.isExists(projectDataModel.excludePackagesList)) {
      projectDataModel.excludePackagesList = projectDataModel.excludePackagesList.map((p) =>
        textUtils.toLowerCaseTrim(p)
      );
    }
    return projectDataModel;
  }
  ...
  validateJSONString(data, jsonData) {
    ...
    if (
      isRequired &&
      !validationUtils.isPropertyExists({
        obj: jsonData,
  ...
  findMatchCustomPackages(projectDataModel) {
    ...
    const { customPackagesList, dependencies, devDependencies, isIncludeDevDependencies } =
      projectDataModel;
    ...
    if (projectDataModel.updateType === UpdateTypeEnum.CUSTOM) {
      if (!validationUtils.isExists(projectDataModel.customPackagesList)) {
        return this.updateProjectStatus({
          ...
          resultMessage:
            'The project update type marked as custom but no custom packages were found (1000033)',
        });
      }
      ...
      if (!this.findMatchCustomPackages(projectDataModel)) {
        return this.updateProjectStatus({
          ...
          resultMessage:
            'No match custom package in the dependencies or devDependencies objects were found (1000034)',
        });
      }
    }
    return projectDataModel;
  ...
  finalizeProject(projectDataModel) {
    ...
    projectDataModel.customPackagesList = textUtils.removeDuplicates(
      projectDataModel.customPackagesList
    );
    projectDataModel.excludePackagesList = textUtils.removeDuplicates(
      projectDataModel.excludePackagesList
    );
  ...
  createPackagesTemplate(projectDataModel) {
    ...
    if (!validationUtils.isExists(projectDataModel.packagesTemplateKeys)) {
      return this.updateProjectStatus({
        ...
        resultMessage:
          'There are no packages to validate. Consider rechecking the custom/exclude lists (1000035)',
      });
    }
  ...
  async getProjectOutdatedPackages(data) {
    ...
    if (errorMessage) {
      ...
      resultMessage = validationUtils.isExists(projectDataModel.outdatedPackagesKeys)
        ? 'Success to handle the outdated packages.'
    ...
    if (
      status === ProjectStatusEnum.SUCCESS &&
      validationUtils.isExists(projectDataModel.outdatedPackagesKeys)
    ) {
  ...
  getProjectsUpdateAvailableCount() {
    return this.projectsDataModel.projectsList.filter(
      (p) => p.isPackagesUpdate && validationUtils.isExists(p.packagesList)
    ).length;
  }
  ...
  async cleanTemporaryDirectory() {
    ...
    for (
      let i = 0;
      i < countLimitService.countLimitDataModel.maximumDeleteTemporaryDirectoryRetriesCount;
      i++
    ) {
      try {
      ...
      if (isCleaned) {
        ...
        await globalUtils.sleep(
          countLimitService.countLimitDataModel.millisecondsTimeoutDeleteTemporaryDirectory
        );
      }
    }
    if (!isCleaned) {
  ...
  async removeTemporaryDirectory() {
    ...
    if (!isRemoved) {
      throw new Error(
        `${pathService.pathDataModel.temporaryDirectoryPath} directory is not removable (1000037)`
      );
    }
  }
  ...
  async updateProjectOutdatedPackages(data) {
    ...
    try {
      ...
      if (
        !isPackagesUpdate ||
        status !== ProjectStatusEnum.SUCCESS ||
        !validationUtils.isExists(packagesList)
      ) {
      ...
      if (
        !parentProjectPath ||
        fileUtils.isDirectoryEmpty(pathService.pathDataModel.temporaryDirectoryPath)
      ) {
      ...
      if (
        projectDataModel.retriesCount <= countLimitService.countLimitDataModel.maximumRetriesCount
      ) {
  ...
  selectUpdateProjects(totalUpdatableProjectsCount) {
    ...
    if (
      totalUpdatableProjectsCount < countLimitService.countLimitDataModel.maximumProjectsUpdateCount
    ) {
      return;
    }
    ...
    projectIndexesList = textUtils.getListRandomElements(
      projectIndexesList,
  ...
  async updateProjectsOutdatedPackages() {
    ...
    for (let i = 0; i < this.projectsDataModel.projectsList.length; i++) {
      ...
      if (isUpdated) {
        ...
        await globalUtils.sleep(
          countLimitService.countLimitDataModel.millisecondsTimeoutUpdateProject
        );
      }
    }
  }
