...
class LogService {
  ...
  createProjectTemplate(data) {
    const {
      displayName,
      packagesTemplateKeys,
      outdatedPackagesKeys,
      packagesList,
      status,
      resultMessage,
    } = data;
    const lines = [];
    const displayPackageName = `${displayName} ${textUtils.addLeadingZero(
      outdatedPackagesKeys?.length
    )}/${textUtils.addLeadingZero(packagesTemplateKeys?.length)}`;
    ...
    logUtils.logProgress({
      progressData: {
        [`WORKING (${applicationService.applicationDataModel.displayStatus}${
          retriesCount ? ` | RETRIES: ${retriesCount}` : ''
        })`]: `${displayName} ${textUtils.getNumberOfNumber({
