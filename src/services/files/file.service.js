...
class FileService {
  ...
  async getFileData(data) {
    ...
    if (!(await fileUtils.isPathExists(filePath))) {
      fileDataResultModel.errorMessage = `Path not exists: ${filePath} (1000004)`;
      return fileDataResultModel;
    }
    if (!fileUtils.isFilePath(filePath)) {
      fileDataResultModel.errorMessage = `The parameter path ${parameterName} marked as file but it's a path of a directory: ${filePath} (1000005)`;
      return fileDataResultModel;
    }
    const extension = pathUtils.getExtension(filePath);
    if (extension !== fileExtension) {
      fileDataResultModel.errorMessage = `The parameter path ${parameterName} must be a ${fileExtension} file but it's: ${extension} file (1000006)`;
      return fileDataResultModel;
    }
    ...
    if (!isPackageJSONFile && !validationUtils.isExists(fileDataResultModel.resultData)) {
      fileDataResultModel.errorMessage = `No data exists in the file: ${filePath} (1000007)`;
      return fileDataResultModel;
    }
    return fileDataResultModel;
  }
