...
class PackageService {
  ...
  async validatePackageJsonUpdates(data) {
    ...
    const packageJson = await fileUtils.read(packageJsonPath);
    for (let i = 0; i < packagesList.length; i++) {
      packagesList[i].status =
        packageJson.indexOf(packagesList[i].outdatedPackage) === -1 &&
        packageJson.indexOf(packagesList[i].updatePackage) > -1
          ? PackageStatusEnum.UPDATED
  ...
  updatePackagesStatus(data) {
    ...
    for (let i = 0; i < packagesList.length; i++) {
      packagesList[i].status = isErrorExists
        ? PackageStatusEnum.FAILED
        : PackageStatusEnum.GIT_PUSHED;
    }
    return packagesList;
  }
}
