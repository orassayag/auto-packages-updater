...
class InitiateService {
  ...
  validateScriptType() {
    if (
      ...
      throw new Error('Invalid or no ScriptTypeEnum parameter was found (1000008)');
    }
  }
  ...
  validateSettings() {
    ...
    if (!settings) {
      throw new Error('Invalid or no settings object was found (1000009)');
    }
  ...
  calculateSettings() {
    ...
    settings.TEMPORARY_DIRECTORY_PATH = pathUtils.getJoinPath({
      targetPath: settings.DIST_PATH,
      targetName: TEMPORARY_DIRECTORY_PATH,
    });
  }
  ...
  validatePositiveNumbers() {
    [
      ...
      if (!validationUtils.isPositiveNumber(value)) {
        throw new Error(
          `Invalid or no ${key} parameter was found: Expected a number but received: ${value} (1000010)`
        );
      }
    });
  }
  ...
  validateStrings() {
    ...
    [
      ...
      'GITHUB_URL',
      // ===LOG=== //
      'DIST_OUTDATED_FILE_NAME',
      'DIST_UPDATED_FILE_NAME',
      // ===SOURCE=== //
      'PROJECTS_PATH',
      // ===ROOT PATH=== //
      'APPLICATION_NAME',
      ...
      if (!validationUtils.isExists(value)) {
        throw new Error(
          `Invalid or no ${key} parameter was found: Expected a string but received: ${value} (1000011)`
        );
      }
    });
  }
  ...
  validateBooleans() {
    [
      ...
      'IS_AUTO_UPDATE',
      'IS_LOG_ONLY_UPDATES',
      'IS_SIMULATE_UPDATE_MODE',
    ].map((key) => {
      const value = settings[key];
      if (!validationUtils.isValidBoolean(value)) {
        throw new Error(
          `Invalid or no ${key} parameter was found: Expected a boolean but received: ${value} (1000012)`
        );
      }
    });
  }
  ...
  validateArrays() {
    [
      ...
      if (!validationUtils.isValidArray(value)) {
        throw new Error(
          `Invalid or no ${key} parameter was found: Expected a array but received: ${value} (1000013)`
        );
      }
    });
  }
  ...
  validateSpecial() {
    ...
    if (!fileUtils.isFilePath(PROJECTS_PATH)) {
      throw new Error(
        `The path PROJECTS_PATH parameter needs to be a file path but it's a directory path: ${PROJECTS_PATH} (1000014)`
      );
    }
    [
      // ===GENERAL=== //
      ...
      if (!validationUtils.isValidLink(value)) {
        throw new Error(
          `Invalid or no ${key} parameter was found: Expected a valid URL but received: ${value} (1000015)`
        );
      }
    });
  }
  ...
  validateDirectories() {
    ...
    [
      ...
      if (!fileUtils.isDirectoryPath(value)) {
        throw new Error(
          `The parameter path ${key} marked as directory but it's a path of a file: ${value} (1000016)`
        );
      }
    });
  }
