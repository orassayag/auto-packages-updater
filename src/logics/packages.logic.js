...
class PackagesLogic {
  ...
  async startSession() {
    ...
    const isUpdatedPackages =
      applicationService.applicationDataModel.isAutoUpdate &&
      projectService.getProjectsUpdateAvailableCount();
    if (isUpdatedPackages) {
      ...
      this.updateStatus(
        ...
        StatusEnum.UPDATE_PARENT,
        DisplayStatusEnum.UPDATE_PARENT,
        true
      );
  ...
  async sleep() {
    await globalUtils.sleep(
      countLimitService.countLimitDataModel.millisecondsTimeoutExitApplication
    );
  }
